...
import com.e1858.adapter.ListViewKnowledgeAdapter;
...
public class KnowlegeActivity extends BaseMenuActivity {	
	private PullToRefreshListView 		knowlege_list;	
	...
	private ListViewKnowledgeAdapter  	adapter;	
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		baseSetContentView(R.layout.knowlege);	
		doScroll(ModuleInteger.KNOWLEGE);	
		initView();
		loadGroupData();
	}
	private void initView(){
		
		cEappApp.dbManager.createLibrary(TableName.LIBRARY+ModuleInteger.KNOWLEGE);
		cEappApp.dbManager.createLibraryGroup(TableName.LIBRARY_GROUP+ModuleInteger.KNOWLEGE);
		title_name.setText("知识库");
	...
	private void initknowlegeListView(){
		knowlege_list=(PullToRefreshListView)findViewById(R.id.knowlege_list);
		...
		knowlege_list.addFooterView(footer_view);
		knowlege_list.setOnScrollListener(new AbsListView.OnScrollListener() {
				public void onScrollStateChanged(AbsListView view, int scrollState) {
					knowlege_list.onScrollStateChanged(view, scrollState);
					//数据为空--不用继续下面代码了
				...
				public void onScroll(AbsListView view, int firstVisibleItem,int visibleItemCount, int totalItemCount) {
					knowlege_list.onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
				}
			});
			knowlege_list.setOnRefreshListener(new PullToRefreshListView.OnRefreshListener() {
	...
	public void changPopState(View v) { 		
			 ...
			 group_list.setOnItemClickListener(new OnItemClickListener() {
				 ...
				 public void onItemClick(AdapterView<?> arg0, View view,
					...
					// TODO Auto-generated method stub
					 ...
					 try{
						 Cursor c=cEappApp.dbManager.queryData2Cursor("select * from "+ TableName.LIBRARY+ModuleInteger.KNOWLEGE +" where groupID=?", new String[]{""+groupID});
						 Log.v("c.getcount", "==="+c.getCount());
						 if(c.getCount()==0){
							 if(NetUtil.checkNetWorkStatus(KnowlegeActivity.this)){
	 ...
	 private void loadlocalData(){
		 try{
			 Cursor c=cEappApp.dbManager.queryData2Cursor("select *  from "+TableName.LIBRARY_GROUP+ModuleInteger.KNOWLEGE, null);
			 if(c.getCount()==0){
				 Cursor cursor=cEappApp.dbManager.queryData2Cursor("select * from "+TableName.LIBRARY+ModuleInteger.KNOWLEGE, null);
	 ...
	 private void loadlocalLibData(long groupID){
		 try{
			 Cursor c=cEappApp.dbManager.queryData2Cursor("select * from "+TableName.LIBRARY+ModuleInteger.KNOWLEGE+" where groupID=? order by id desc ", new String[]{""+groupID});
			 ...
			 if(libs.size()==0){
				 knowlege_list.setVisibility(View.GONE);
				 empty_text.setVisibility(View.VISIBLE);
				 empty_text.setText("很抱歉,暂无数据！");
			 }else{
				 knowlege_list.setVisibility(View.VISIBLE);
				 empty_text.setVisibility(View.GONE);
				 adapter=new ListViewKnowledgeAdapter(this, libs, R.layout.knowlege_item);
				 knowlege_list.setAdapter(adapter);
			 }
	...
	private void loadGroupData(){
		if(NetUtil.checkNetWorkStatus(this)){
			...
			getLibraryGroupList.setModule(ModuleInteger.KNOWLEGE);
			...
			case MessageWhat.GET_LIBRARYGROUP_LIST_RESP:
				if(null!=(String)msg.obj){
					...
					if(HttpDefine.RESPONSE_SUCCESS==getLibraryGroupListResp.getStatus()){
						...
						cEappApp.dbManager.deleteData(TableName.LIBRARY_GROUP+ModuleInteger.KNOWLEGE, null, null);
						for(int i=0;i<grouplist.size();i++){	
							...
							cEappApp.dbManager.insertData(TableName.LIBRARY_GROUP+ModuleInteger.KNOWLEGE, values);
						}
				...
				if(null!=(String)msg.obj){
					...
					if(HttpDefine.RESPONSE_SUCCESS==getLibraryListResp.getStatus()){
						...
						if((serverlibs==null||serverlibs.size()==0)&&direction==DirectionType.NEW){
							knowlege_list.onRefreshComplete(getString(R.string.pull_to_refresh_update) + new Date().toLocaleString());
						...
						if(direction==DirectionType.NEW){
							...
							if(serverlibs.size()==pageSize){
								try{
									cEappApp.dbManager.deleteData(TableName.LIBRARY+ModuleInteger.KNOWLEGE , "groupID=?", new String[]{""+groupID});
								}catch(Exception e){
									e.printStackTrace();
								}
							}
							for(int i=0;i<serverlibs.size();i++){
								...
								cEappApp.dbManager.insertData(TableName.LIBRARY+ModuleInteger.KNOWLEGE, values);
							}
						 ...
						 if(libs.size()==0){
							 ...
							 if(direction==DirectionType.OLD){
									knowlege_list.setSelection(knowlege_list.getCount()-serverlibs.size()+1);
								}
						 }
