...
public class ListViewMessageAdapter extends BaseAdapter {
	...
	public ListViewMessageAdapter(MessageActivity context,List<Msg> list,int itemResource){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		itemView.title.setText(list.get(position).getSender().getRealName());
	
		try{
			itemView.time.setText(DateUtil.getAccurateTime(Constant.SIMPLE_DATE_FORMAT.parse(DateUtil.strToDatestrLong(list.get(position).getTime().toString()))));
		...
		if(list.get(position).getUnread()==IntentValue.UNREAD){
			itemView.title.setTextColor(context.getResources().getColor(R.color.orange_bg));
		}
		if(allSelect){
